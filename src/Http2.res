...
module Http2Session = {
  ...
  @send
  external ping: (t, (Js.Nullable.t<JsExn.t>, int, Buffer.t) => unit) => bool = "ping"
  @send
  external pingWith: (
    t,
    ~payload: Buffer.t,
    (Js.Nullable.t<JsExn.t>, int, Buffer.t) => unit,
  ) => bool = "ping"
  ...
  external settings: (
    t,
    settingsObject,
    ~callback: (Js.Null.t<JsExn.t>, settingsObject, int) => unit=?,
  ) => unit = "settings"
