...
external fromSymbol: Symbol.t => t<'a => 'b, 'ty> = "%identity"
external fromSymbol2: Symbol.t => t<('a, 'b) => 'c, 'ty> = "%identity"
external fromSymbol3: Symbol.t => t<('a, 'b, 'c) => 'd, 'ty> = "%identity"
...
external unsafeToSymbol: t<'a => 'b, 'ty> => Symbol.t = "%identity"
external unsafeToSymbol2: t<('a, 'b) => 'c, 'ty> => Symbol.t = "%identity"
external unsafeToSymbol3: t<('a, 'b, 'c) => 'd, 'ty> => Symbol.t = "%identity"
type case =
  | String(string)
  | Symbol(Symbol.t)
  | Unknown
...
let eq = (event1, event2) =>
  switch (typeof(event1), typeof(event2)) {
